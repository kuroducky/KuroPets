...
const createChat = (request, response) => {
    const id1 = parseInt(request.params.accID1)
    const id2 = parseInt(request.params.accID2)
    pool.query('INSERT INTO "tbl_Chat" ("sendeeID", "senderID", "chatMessages", "chatTime") VALUES ($1, $2, $3, current_timestamp) RETURNING *',
    [id1, id2, `{${request.body.chatMessage}}`],
    (error, results) => {
        if(error){
            throw error
        }
        response.status(200).send(`Chat created with ID: ${results.rows[0].chatID}`)
    })
}
