...
const updatePost = (req, res) => {
    ...
    pool.connect(function(error, client, done){
        if(error){
            ...
            return log.error('Could not connect to pgsql server', error);
        }

        client.query('BEGIN', function(error){
            if(error){
                console.error('error', error.stack);
                log.error('Problem starting transaction', error);
                return done(true);
            }
            ...
            function(error, result){
                if(error){
                    log.error('Unable to update data, rolling back transaction', error);
                    return client.query('ROLLBACK', function(error){
                        if(error){
                            log.error('Unable to rollback transaction, killing client', error);
                        }
                        done(error);
                    });
                }
                ...
                client.query('COMMIT', function(error){
                    if(error){
                        log.error('Unable to commit transaction, killing client', error);
                    }
                    done(error);
                })
            })
